...
# --- 3. KI System-Prompt ---
# Das LLM kümmert sich NUR um die unstrukturierten Daten
SYSTEM_PROMPT = """
...
def verarbeite_telegram_nachricht(telegram_text, telegram_timestamp, sender_name):
    """Schickt den Text an Llama 3.1 und ergänzt ALLE Felder des Datenbankschemas"""
    ...
    # 3. Python als Manager: Generiert die restlichen Pflichtfelder
    station = llm_daten.get('station') or 'unbekannt'
    station_clean = station.replace(" ", "")
    linie = llm_daten.get('linie') or 'unbekannt'
    
    # Generiere Unique ID nach Schema: {station}-{linie}-{date}-{time}
    date_str = telegram_timestamp.strftime('%Y%m%d')
    time_str = telegram_timestamp.strftime('%H%M')
    llm_daten['event_id'] = f"{station_clean}-{linie}-{date_str}-{time_str}"
    
    # Ergänze die von Python erfassten Metadaten
    llm_daten['gemeldet_um'] = telegram_timestamp.isoformat()
    llm_daten['beendet_um'] = None        # Laut Schema null, solange aktiv
    llm_daten['dauer_minuten'] = None     # Laut Schema vorerst null
    llm_daten['raw_message'] = telegram_text
    llm_daten['telegram_sender'] = sender_name
...
async def new_message_handler(event):
    ...
    # Absendername auslesen (für das Feld telegram_sender)
    ...
    try:
        # Nachricht verarbeiten und den Absendernamen mitgeben
        fertiges_json = verarbeite_telegram_nachricht(text, timestamp, sender_name)
